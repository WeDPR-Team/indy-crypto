...
#[no_mangle]
pub extern "C" fn indy_crypto_cl_prover_new_master_secret(
    ...
    let res = match Prover::new_master_secret() {
        Ok(master_secret) => {
            trace!(
                "indy_crypto_cl_prover_new_master_secret: master_secret: {:?}",
                master_secret
            );
            unsafe {
                *master_secret_p = Box::into_raw(Box::new(master_secret)) as *const c_void;
                trace!(
                    "indy_crypto_cl_prover_new_master_secret: *master_secret_p: {:?}",
                    *master_secret_p
                );
            }
            ErrorCode::Success
    ...
    trace!(
        "indy_crypto_cl_prover_new_master_secret: <<< res: {:?}",
        res
    );
    res
}
...
#[no_mangle]
pub extern "C" fn indy_crypto_cl_master_secret_to_json(
    master_secret: *const c_void,
    master_secret_json_p: *mut *const c_char,
) -> ErrorCode {
    trace!(
        "indy_crypto_cl_master_secret_to_json: >>> master_secret: {:?}, master_secret_json_p: {:?}",
        master_secret,
    ...
    trace!(
        "indy_crypto_cl_master_secret_to_json: entity >>> master_secret: {:?}",
        master_secret
    );
    ...
    let res = match serde_json::to_string(master_secret) {
        Ok(master_secret_json) => {
            trace!(
                "indy_crypto_cl_master_secret_to_json: master_secret_json: {:?}",
                master_secret_json
            );
            unsafe {
                ...
                *master_secret_json_p = master_secret_json.into_raw();
                trace!(
                    "indy_crypto_cl_master_secret_to_json: master_secret_json_p: {:?}",
                    *master_secret_json_p
                );
            }
            ErrorCode::Success
...
/// * `master_secret_p` - Reference that will contain master secret instance pointer.
#[no_mangle]
pub extern "C" fn indy_crypto_cl_master_secret_from_json(
    ...
    trace!(
        "indy_crypto_cl_master_secret_from_json: entity: master_secret_json: {:?}",
        master_secret_json
    );
    ...
    let res = match serde_json::from_str::<MasterSecret>(&master_secret_json) {
        Ok(master_secret) => {
            trace!(
                "indy_crypto_cl_master_secret_from_json: master_secret: {:?}",
                master_secret
            );
            unsafe {
                *master_secret_p = Box::into_raw(Box::new(master_secret)) as *const c_void;
                trace!(
                    "indy_crypto_cl_master_secret_from_json: *master_secret_p: {:?}",
                    *master_secret_p
                );
            }
            ErrorCode::Success
...
/// * `master_secret` - Reference that contains master secret instance pointer.
#[no_mangle]
pub extern "C" fn indy_crypto_cl_master_secret_free(master_secret: *const c_void) -> ErrorCode {
    trace!(
        "indy_crypto_cl_master_secret_free: >>> master_secret: {:?}",
        master_secret
    );

    check_useful_c_ptr!(master_secret, ErrorCode::CommonInvalidParam1);

    let master_secret = unsafe {
        Box::from_raw(master_secret as *mut MasterSecret);
    };
    trace!(
        "indy_crypto_cl_master_secret_free: entity: master_secret: {:?}",
        master_secret
    );
...
pub extern "C" fn indy_crypto_cl_prover_blind_credential_secrets(
    ...
    trace!(
        "indy_crypto_cl_prover_blind_credential_secrets: >>> credential_pub_key: {:?}, \
    ...
    trace!(
        "indy_crypto_cl_prover_blind_credential_secrets: inputs: credential_pub_key: {:?}, \
         credential_key_correctness_proof: {:?}, \
         credential_values: {:?}, \
         credential_nonce: {:?}",
        credential_pub_key,
    ...
    trace!(
        "indy_crypto_cl_prover_blind_credential_secrets: <<< res: {:?}",
        res
    );
    res
}
...
#[no_mangle]
pub extern "C" fn indy_crypto_cl_blinded_credential_secrets_to_json(
    blinded_credential_secrets: *const c_void,
    blinded_credential_secrets_json_p: *mut *const c_char,
) -> ErrorCode {
    trace!(
        "indy_crypto_cl_blinded_credential_secrets_to_json: >>> blinded_credential_secrets: {:?}\n\
        ...
        Ok(blinded_credential_secrets_json) => {
            ...
            unsafe {
                let blinded_credential_secrets_json =
                    CTypesUtils::string_to_cstring(blinded_credential_secrets_json);
    ...
    trace!(
        "indy_crypto_cl_blinded_credential_secrets_to_json: <<< res: {:?}",
        res
    );
    res
}
...
#[no_mangle]
pub extern "C" fn indy_crypto_cl_blinded_credential_secrets_from_json(
    ...
    trace!(
        "indy_crypto_cl_blinded_credential_secrets_from_json: <<< res: {:?}",
        res
    );
    res
}
...
#[no_mangle]
pub extern "C" fn indy_crypto_cl_blinded_credential_secrets_free(
    blinded_credential_secrets: *const c_void,
) -> ErrorCode {
    trace!(
        "indy_crypto_cl_blinded_credential_secrets_free: >>> blinded_credential_secrets: {:?}",
        blinded_credential_secrets
    );

    check_useful_c_ptr!(blinded_credential_secrets, ErrorCode::CommonInvalidParam1);

    let blinded_credential_secrets = unsafe {
        Box::from_raw(blinded_credential_secrets as *mut BlindedCredentialSecrets);
    };
    trace!(
        "indy_crypto_cl_blinded_credential_secrets_free: entity: blinded_credential_secrets: {:?}",
        blinded_credential_secrets
    );
    ...
    trace!(
        "indy_crypto_cl_blinded_credential_secrets_free: <<< res: {:?}",
        res
    );
    res
}
...
#[no_mangle]
pub extern "C" fn indy_crypto_cl_credential_secrets_blinding_factors_to_json(
        ...
        Ok(credential_secrets_blinding_factors_json) => {
            ...
            unsafe {
                let credential_secrets_blinding_factors_json =
                    CTypesUtils::string_to_cstring(credential_secrets_blinding_factors_json);
                *credential_secrets_blinding_factors_json_p =
    ...
    trace!(
        "indy_crypto_cl_credential_secret_blinding_factors_to_json: <<< res: {:?}",
        res
    );
    res
}
...
/// * `credential_secrets_blinding_factors_p` - Reference that will contain credential secrets blinding factors instance pointer.
#[no_mangle]
pub extern "C" fn indy_crypto_cl_credential_secrets_blinding_factors_from_json(
    ...
    trace!(
        "indy_crypto_cl_credential_secrets_blinding_factors_from_json: <<< res: {:?}",
        res
    );
    res
}
...
#[no_mangle]
pub extern "C" fn indy_crypto_cl_credential_secrets_blinding_factors_free(
    credential_secrets_blinding_factors: *const c_void,
) -> ErrorCode {
    ...
    trace!(
        "indy_crypto_cl_credential_secrets_blinding_factors_free: <<< res: {:?}",
        res
    );
    res
}
...
#[no_mangle]
pub extern "C" fn indy_crypto_cl_blinded_credential_secrets_correctness_proof_to_json(
    ...
    trace!(
        "indy_crypto_cl_blinded_credential_secrets_correctness_proof_to_json: <<< res: {:?}",
        res
    );
    res
}
...
#[no_mangle]
pub extern "C" fn indy_crypto_cl_blinded_credential_secrets_correctness_proof_from_json(
    ...
    trace!(
        "indy_crypto_cl_blinded_credential_secrets_correctness_proof_from_json: <<< res: {:?}",
        res
    );
    res
}
...
#[no_mangle]
pub extern "C" fn indy_crypto_cl_blinded_credential_secrets_correctness_proof_free(
    blinded_credential_secrets_correctness_proof: *const c_void,
) -> ErrorCode {
    ...
    trace!(
        "indy_crypto_cl_blinded_credential_secrets_correctness_proof_free: <<< res: {:?}",
        res
    );
    res
}
...
pub extern "C" fn indy_crypto_cl_prover_process_credential_signature(
    ...
    trace!(
        "indy_crypto_cl_prover_process_credential_signature: >>> credential_signature: {:?}\n\
    ...
    trace!(
        "indy_crypto_cl_prover_process_credential_signature: <<< res: {:?}",
        res
    );
    ErrorCode::Success
}
...
pub extern "C" fn indy_crypto_cl_prover_get_credential_revocation_index(
    ...
    trace!(
        "indy_crypto_cl_prover_get_credential_revocation_index: >>> credential_signature: {:?}",
        credential_signature
    );
    ...
    let res = match credential_signature.extract_index() {
        Some(index) => {
            trace!(
                "indy_crypto_cl_prover_get_credential_revocation_index: index: {:?}",
                index
            );
            unsafe {
                *cred_rev_indx = index;
            ...
            trace!(
                "indy_crypto_cl_prover_get_credential_revocation_index: *cred_rev_indx: {:?}",
                cred_rev_indx
            );
            ErrorCode::Success
        }
    ...
    trace!(
        "indy_crypto_cl_prover_get_credential_revocation_index: <<< res: {:?}",
        res
    );
    ErrorCode::Success
}
...
#[no_mangle]
pub extern "C" fn indy_crypto_cl_prover_new_proof_builder(
    ...
    let res = match Prover::new_proof_builder() {
        Ok(proof_builder) => {
            trace!(
                "indy_crypto_cl_prover_new_proof_builder: proof_builder: {:?}",
                proof_builder
            );
            unsafe {
                *proof_builder_p = Box::into_raw(Box::new(proof_builder)) as *const c_void;
                trace!(
                    "indy_crypto_cl_prover_new_proof_builder: *proof_builder_p: {:?}",
                    *proof_builder_p
                );
            }
            ErrorCode::Success
    ...
    trace!(
        "indy_crypto_cl_prover_new_proof_builder: <<< res: {:?}",
        res
    );
    res
}
...
pub extern "C" fn indy_crypto_cl_proof_builder_add_sub_proof_request(
    ...
    trace!(
        "indy_crypto_cl_proof_builder_add_sub_proof_request: entities: proof_builder: {:?}, \
         sub_proof_request: {:?}, \
         credential_schema: {:?}, \
         non_credential_schema: {:?}, \
         credential_signature: {:?}, \
    ...
    trace!(
        "indy_crypto_cl_proof_builder_add_sub_proof_request: <<< res: {:?}",
        res
    );
    ErrorCode::Success
}
...
pub extern "C" fn indy_crypto_cl_proof_builder_finalize(
    ...
    trace!(
        "indy_crypto_cl_proof_builder_finalize: entities: proof_builder: {:?}, nonce: {:?}",
        proof_builder,
        nonce
    );
        ...
        Ok(proof) => {
            ...
            unsafe {
                *proof_p = Box::into_raw(Box::new(proof)) as *const c_void;
                trace!(
                    "indy_crypto_cl_proof_builder_finalize: *proof_p: {:?}",
                    *proof_p
                );
            }
            ErrorCode::Success
...
#[no_mangle]
pub extern "C" fn indy_crypto_cl_proof_to_json(
    proof: *const c_void,
    proof_json_p: *mut *const c_char,
) -> ErrorCode {
    trace!(
        "indy_crypto_cl_proof_to_json: >>> proof: {:?}, proof_json_p: {:?}",
        proof,
        proof_json_p
    );
    ...
    trace!(
        "indy_crypto_cl_proof_to_json: entity >>> proof: {:?}",
        proof
    );
        ...
        Ok(proof_json) => {
            ...
            unsafe {
                ...
                *proof_json_p = proof_json.into_raw();
                trace!(
                    "indy_crypto_cl_proof_to_json: proof_json_p: {:?}",
                    *proof_json_p
                );
            }
            ErrorCode::Success
...
/// * `proof_p` - Reference that will contain proof instance pointer.
#[no_mangle]
pub extern "C" fn indy_crypto_cl_proof_from_json(
    proof_json: *const c_char,
    proof_p: *mut *const c_void,
) -> ErrorCode {
    trace!(
        "indy_crypto_cl_proof_from_json: >>> proof_json: {:?}, proof_p: {:?}",
        proof_json,
        proof_p
    );
    ...
    trace!(
        "indy_crypto_cl_proof_from_json: entity: proof_json: {:?}",
        proof_json
    );
...
pub extern "C" fn indy_crypto_cl_proof_free(proof: *const c_void) -> ErrorCode {
    ...
    let proof = unsafe {
        Box::from_raw(proof as *mut Proof);
    };
...
mod tests {
    ...
    fn indy_crypto_cl_master_secret_to_json_works() {
        ...
        let err_code =
            indy_crypto_cl_master_secret_to_json(master_secret, &mut master_secret_json_p);
        assert_eq!(err_code, ErrorCode::Success);
    ...
    fn indy_crypto_cl_master_secret_from_json_works() {
        ...
        let mut master_secret_p: *const c_void = ptr::null();
        let err_code =
            indy_crypto_cl_master_secret_from_json(master_secret_json_p, &mut master_secret_p);
        assert_eq!(err_code, ErrorCode::Success);
    ...
    fn indy_crypto_cl_prover_blind_credential_secrets_works() {
        ...
        let err_code = indy_crypto_cl_prover_blind_credential_secrets(
            credential_pub_key,
            credential_key_correctness_proof,
            credential_values,
            credential_nonce,
    ...
    fn indy_crypto_cl_prover_blinded_credential_secrets_free_works() {
        ...
        let err_code = indy_crypto_cl_credential_secrets_blinding_factors_free(
            credential_secrets_blinding_factors,
        );
        assert_eq!(err_code, ErrorCode::Success);

        let err_code = indy_crypto_cl_blinded_credential_secrets_correctness_proof_free(
            blinded_credential_secrets_correctness_proof,
        );
        assert_eq!(err_code, ErrorCode::Success);
    ...
    fn indy_crypto_cl_prover_blinded_credential_secrets_to_json_works() {
        ...
        let err_code = indy_crypto_cl_blinded_credential_secrets_to_json(
            blinded_credential_secrets,
    ...
    fn indy_crypto_cl_prover_proof_builder_add_sub_proof_request_works() {
        ...
        let err_code = indy_crypto_cl_proof_builder_add_sub_proof_request(
            proof_builder,
            sub_proof_request,
            credential_schema,
            non_credential_schema,
    ...
    fn indy_crypto_cl_prover_blinded_credential_secrets_from_json_works() {
        ...
        let mut blinded_credential_secrets_p: *const c_void = ptr::null();
        let err_code = indy_crypto_cl_blinded_credential_secrets_from_json(
            blinded_credential_secrets_json_p,
            &mut blinded_credential_secrets_p,
        );
        assert_eq!(err_code, ErrorCode::Success);
    ...
    fn indy_crypto_cl_prover_credential_secrets_blinding_factors_to_json_works() {
        ...
        let err_code = indy_crypto_cl_credential_secrets_blinding_factors_to_json(
            credential_secrets_blinding_factors,
    ...
    fn indy_crypto_cl_prover_credential_secrets_blinding_factors_from_json_works() {
        ...
        let mut credential_secrets_blinding_factors_p: *const c_void = ptr::null();
        let err_code = indy_crypto_cl_credential_secrets_blinding_factors_from_json(
            credential_secrets_blinding_factors_json_p,
            &mut credential_secrets_blinding_factors_p,
        );
        assert_eq!(err_code, ErrorCode::Success);
    ...
    fn indy_crypto_cl_prover_blinded_credential_secrets_correctness_proof_to_json_works() {
        ...
        let err_code = indy_crypto_cl_blinded_credential_secrets_correctness_proof_to_json(
            blinded_credential_secrets_correctness_proof,
    ...
    fn indy_crypto_cl_prover_blinded_credential_secrets_correctness_proof_from_json_works() {
        ...
        let mut blinded_credential_secrets_correctness_proof_p: *const c_void = ptr::null();
        let err_code = indy_crypto_cl_blinded_credential_secrets_correctness_proof_from_json(
            blinded_credential_secrets_correctness_proof_json_p,
            &mut blinded_credential_secrets_correctness_proof_p,
        );
        assert_eq!(err_code, ErrorCode::Success);
    ...
    fn indy_crypto_cl_prover_process_credential_signature_signature_works() {
        ...
        let err_code = indy_crypto_cl_prover_process_credential_signature(
            credential_signature,
            credential_values,
            signature_correctness_proof,
            credential_secrets_blinding_factors,
    ...
    fn indy_crypto_cl_prover_get_credential_revocation_index_works() {
        ...
        let err_code = indy_crypto_cl_prover_get_credential_revocation_index(
            credential_signature,
            &mut cred_rev_idx_p,
        );
        assert_eq!(err_code, ErrorCode::Success);
