...
#[no_mangle]
pub extern "C" fn indy_crypto_cl_verifier_new_proof_verifier(
    ...
    let res = match Verifier::new_proof_verifier() {
        Ok(proof_verifier) => {
            trace!(
                "indy_crypto_cl_verifier_new_proof_verifier: proof_verifier: {:?}",
                proof_verifier
            );
            unsafe {
                *proof_verifier_p = Box::into_raw(Box::new(proof_verifier)) as *const c_void;
                trace!(
                    "indy_crypto_cl_verifier_new_proof_verifier: *proof_verifier_p: {:?}",
                    *proof_verifier_p
                );
            }
            ErrorCode::Success
    ...
    trace!(
        "indy_crypto_cl_verifier_new_proof_verifier: <<< res: {:?}",
        res
    );
    res
}
...
#[no_mangle]
pub extern "C" fn indy_crypto_cl_proof_verifier_add_sub_proof_request(
    ...
    trace!(
        "indy_crypto_cl_proof_verifier_add_sub_proof_request: <<< res: {:?}",
        res
    );
    ErrorCode::Success
}
    ...
    let res = match proof_verifier.verify(proof, nonce) {
        Ok(valid) => {
            ...
            unsafe {
                *valid_p = valid;
                trace!(
                    "indy_crypto_cl_proof_verifier_verify: *valid_p: {:?}",
                    *valid_p
                );
            }
            ErrorCode::Success
...
mod tests {
    ...
    fn indy_crypto_cl_proof_verifier_add_sub_proof_request_works() {
        ...
        let err_code = indy_crypto_cl_proof_verifier_add_sub_proof_request(
            proof_verifier,
            sub_proof_request,
            credential_schema,
            non_credential_schema,
    ...
    fn indy_crypto_cl_proof_verifier_verify_works_for_primary_proof() {
        ...
        let mut valid = false;
        let err_code = indy_crypto_cl_proof_verifier_verify(
            proof_verifier,
            proof,
            proof_building_nonce,
            &mut valid,
...
pub mod mocks {
    ...
    pub fn _free_proof_verifier(
        ...
        let mut valid = false;
        let err_code =
            indy_crypto_cl_proof_verifier_verify(proof_verifier, proof, nonce, &mut valid);
        assert_eq!(err_code, ErrorCode::Success);
    }
}
