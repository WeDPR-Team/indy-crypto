...
impl Prover {
    ...
    fn _check_signature_correctness_proof(
        ...
        if let Some((ref attr, _)) =
            cred_values
                .attrs_values
                .iter()
                .find(|&(ref attr, ref value)| {
                    (value.is_known() || value.is_hidden())
                        && !p_pub_key.r.contains_key(attr.clone())
                })
        {
            return Err(IndyCryptoError::InvalidStructure(format!(
                "Value by key '{}' not found in public key",
                attr
            )));
        }
...
impl ProofBuilder {
    ...
    fn _init_ne_proof(
        ...
        cred_values: &CredentialValues,
        predicate: &Predicate,
    ) -> Result<PrimaryPredicateInequalityInitProof, IndyCryptoError> {
    ...
    ) -> Result<PrimaryPredicateInequalityProof, IndyCryptoError> {
        trace!(
