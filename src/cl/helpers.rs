...
pub fn _generate_prime_in_range(
    start: &BigNumber,
    end: &BigNumber,
) -> Result<BigNumber, IndyCryptoError> {
    trace!(
...
pub fn calc_teq(
    ...
    for k in unrevealed_attrs.iter() {
        ...
        let cur_m = m_tilde
            .get(k)
            .ok_or(IndyCryptoError::InvalidStructure(format!(
                "Value by key '{}' not found in m_tilde",
                k
            )))?;
...
pub fn four_squares(delta: u32) -> Result<HashMap<String, BigNumber>, IndyCryptoError> {
...
mod tests {
    ...
    fn four_squares_works() {
        let res = four_squares(107 as u32);
        ...
        let res = four_squares(112 as u32);
        let res_data = res.unwrap();
        ...
        let res = four_squares(253 as u32);
        let res_data = res.unwrap();
        ...
        let res = four_squares(1506099439 as u32);
        let res_data = res.unwrap();
