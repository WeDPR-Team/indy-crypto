...
impl Issuer {
    ...
    > {
        ...
        let cred_rev_pub_key: &CredentialRevocationPublicKey = credential_pub_key
            .r_key
            .as_ref()
    ...
    ) -> Result<(CredentialSignature, SignatureCorrectnessProof), IndyCryptoError> {
        trace!(
    ...
    pub fn revoke_credential<RTA>(
        ...
        rev_idx: u32,
        rev_tails_accessor: &RTA,
    ) -> Result<RevocationRegistryDelta, IndyCryptoError>
    where
        RTA: RevocationTailsAccessor,
    {
        trace!(
    ...
    fn _new_credential_revocation_keys() -> Result<
        (
    ...
    fn _new_revocation_registry(
        ...
        max_cred_num: u32,
        issuance_by_default: bool,
    ) -> Result<RevocationRegistry, IndyCryptoError> {
    ...
    fn _new_revocation_registry_keys(
        cred_rev_pub_key: &CredentialRevocationPublicKey,
        max_cred_num: u32,
    ) -> Result<(RevocationKeyPublic, RevocationKeyPrivate), IndyCryptoError> {
        trace!(
            "Issuer::_new_revocation_registry_keys: >>> cred_rev_pub_key: {:?}, max_cred_num: {:?}",
            cred_rev_pub_key,
            max_cred_num
    ...
    fn _check_blinded_credential_secrets_correctness_proof(
        ...
        let u_cap = blinded_cred_secrets.hidden_attributes.iter().fold(
            ...
            |acc, attr| {
                let pk_r = cred_pr_pub_key.r.get(&attr.clone()).ok_or(
                    IndyCryptoError::InvalidStructure(format!(
                        "Value by key '{}' not found in cred_pr_pub_key.r",
                        attr
                    )),
                )?;
    ...
    fn _gen_credential_context(
        ...
        let rev_idx = rev_idx.map(|i| i as i64).unwrap_or(-1);
    ...
    ) -> Result<(PrimaryCredentialSignature, BigNumber), IndyCryptoError> {
    ...
    fn _new_signature_correctness_proof(
        ...
        q: &BigNumber,
        nonce: &BigNumber,
    ) -> Result<SignatureCorrectnessProof, IndyCryptoError> {
    ...
    > {
        ...
        let r_pub_key: &CredentialRevocationPublicKey =
            cred_pub_key
                ...
                .as_ref()
                .ok_or(IndyCryptoError::InvalidStructure(format!(
                    "No revocation part present in credential revocation public key."
                )))?;
